...
\vspace{-1.7em}
\begin{center}
 \href{https://github.com/beusy/applied-machine-learning-in-physics}{\texttt{GitHub}}
\end{center}

\section{Data and exploratory analysis}

The given data is a UCI Real Estate Valuation, and contains 414 property
transactions from Sindian District, New Taipei City, Taiwan \cite{uci}. I converted
the supplied XLSX file to CSV directly in the Python notebook. \texttt{pandas.read\_excel()} loaded the
spreadsheet and \texttt{DataFrame.to\_csv()} saved it. The
...
\toprule
Variable & Meaning and unit & Range \\
\midrule
...
The target distribution in Fig.~\ref{fig:eda} is slightly right-skewed. Most prices are
between about 20 and 60, however there is one observation near 118 which is rather far from the rest. House
age forms several clusters. MRT distance is also strongly right-skewed: most houses
are relatively close to a station, but there a few are 4--6.5 km away. The scales are also
notably different, house age reaches about 44, whereas MRT distance reaches about
6488, which becomes important for our gradient descent in the future parts.
...
from 7.6 to 117.5. Clearly, the house age alone is not a strong predictor: inexpensive
...
minimum, and each subsequent update grows larger.
...
117.5 and the restriction to a linear relationship remain clear limitations.
...
In a more careful version, I would investigate unusual observations, add more features, and test nonlinear terms. I would also
reserve held-out data for evaluation, since the losses reported here measure fit to
the same observations used for training and do not establish performance on new unseen
houses.
...
met this criterion after 5,268 epochs. The agreement confirms that the implemented
gradient converges to the least-squares solution.

\section{AI use}
I used ChatGPT to help me with some of the coding commands. In particular, to refresh my memory on \texttt{pandas} and \texttt{matplotlib}. I also used it to do a final organisation of my notebook and the overleaf document.
